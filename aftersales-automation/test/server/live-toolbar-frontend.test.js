...
test('等待重查卡片只为显式拦截件保留人工提前拒绝按钮', () => {
  ...
  assert.match(appJs, /const canExecute = !executed && !manualArchiveOnly\s+&& sim\.decision\.humanTriggeredExecutionAllowed !== false\s+&& \(item\.status !== 'waiting' \|\| isWaitingIntercept\)/);
...
test('自动执行工单明细缺失时不误报待推理，也不引导运行 legacy collect', () => {
  assert.match(appJs, /\['auto_executed', 'auto_executing'\]\.includes\(item\.status\)\s*\? ''/);
  assert.match(appJs, /执行状态已保留，但推理明细缺失/);
  assert.doesNotMatch(appJs, /node collect\.js --sim/);
});
