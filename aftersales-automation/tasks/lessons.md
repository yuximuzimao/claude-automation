...
### 商责和换货是操作约束，不是事实采集的短路条件

- **现象**：换货工单已有退货单号，但采集器跳过商品对应表/档案；商责又在推理入口直接返回人工，导致系统知道“要人工”，却无法告诉人工实际退回了什么、是否可以同意。
- **根因**：把风险约束和事实判断写成同一条前置优先级，风险标签提前吃掉了退回商品核验。
- **规则**：先判断数据是否可信，再采集退回事实，最后叠加操作约束。换货或商责有退货单号时继续做严格规格核验；核对一致可推荐人工同意，并设置 `requiresHumanReview + autoExecutionBlocked` 排除扫描中的无人自动执行。人工核对后，只有 `humanTriggeredExecutionAllowed: true` 且动作明确的工单才可使用系统单笔或人工发起的批量执行。商责换货必须同时显示两个风险标签。
