 ...
 *
 * 基于 docs/ 规则体系，对采集到的工单数据做自动判断。
 * 只输出 decision 对象，不修改任何数据库。
 *
 * decision.steps 字段记录完整推理路径：
 *   { type: 'read'|'check'|'branch', label?, value?, condition?, result?, text? }
 */
...
function inferDecision(sim, queueItem) {
  const steps = [];
  // s() 记录一个推理步骤并返回自身，方便链式
  function s(step) { steps.push(step); return step; }
  // 最终包装：把 steps 附加到 decision
  ...
  if (workOrderStatus && ['已退款', '退款成功', '已完成退款', '已关闭', '已撤销', '已取消', '用户已取消'].some(ts => workOrderStatus.includes(ts))) {
  ...
  if (type === '仅退款') {
    ...
    if (isShipped) {
      ...
      if (safeToWait === true) {
        s({ type: 'branch', text: `自动标记等待重查 → 在途拦截件，剩余${remainingHours}h > 下次扫描${hoursUntilNextScan.toFixed(1)}h` });
        return fin({
          ...escalate(
  ...
  if (type === '退货退款') {
    ...
    if (!returnTracking) {
      ...
      // NON_MERCHANT_REASONS 定义在 constants.js
      const isNonQualityReason = NON_MERCHANT_REASONS.some(kw => reason.includes(kw));

      // 售后原因是"质量问题"但buyerRemark含超期/无理由关键词（实为个人原因）
      const isQualityWithOverdueRemark = reason.includes('质量问题') &&
        ['买重复', '买多', '买多了', '买错', '拍错', '重复购买', '不想要', '拍多', '未拆封'].some(kw => remark.includes(kw));

      if (isNonQualityReason || isQualityWithOverdueRemark) {
        const rejectNote = isQualityWithOverdueRemark
          ? `售后原因为质量问题但备注"${remark}"，实为超售后期无理由退货，不支持`
    ...
    if (!hasConfirmedReceipt) {
      ...
      s({ type: 'branch', text: '上报 → 退货快递单存在但货物状态非「已收到退货」，尚未入库' });
      return fin(escalate('退货尚未入库确认，需人工核查'));
    }
