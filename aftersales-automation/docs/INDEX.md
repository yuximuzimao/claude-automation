...
**可恢复错误**（retry，仅限 ERP `viperp.superboss.cc`）：
- CDP 超时、页面加载慢、元素未出现
- ERP 页面渲染慢、搜索结果延迟
→ 处理：retry 最多 3 次，仍失败则上报人工。注意：鲸灵域名（`scrm.jlsupp.com`）的操作不在此列，无论错误类型一律不重试。
...
### 3.6 工单取消与商责补充（2026-05-21）

**商责售后原因**：`MERCHANT_FAULT_REASONS` 在 `lib/constants.js` 中维护。2026-05-21 新增 `'质量问题'`——此前因漏加导致商责工单穿透到非商责流程（fb-1778868858729 / fb-1779092166923）。2026-06-11 新增 `'卖家发错货'`，覆盖鲸灵原因文案，避免 flow-5.1 Step4 核对通过后误进入自动同意退款。如需追加商责关键词，在该数组中新增。
...
- `[∞]` **#77 店铺重新登录状态机**：`hasFile=true + status=unknown` 表示 session 已保存但未扫描验证，只显示「未扫描」，不显示「重新登录」；点「取消」后必须立即进入「取消中...」，同时禁用确认/取消，等 `/relogin-cancel` 返回成功后才能清理确认态并恢复「重新登录」。请求失败则保留确认态供重试，禁止提前恢复按钮后再次启动登录，造成旧登录进程失联。登录页关闭或 port 文件缺失也必须退出确认态，禁止永久卡「确认保存」。2026-07-17。
