...
```js
{
  outerId:    string,   // 商品档案V2返回的主商家编码；普通单品严格核对时作为实际编码
...
业务计算时普通商品 `subItemNum=0` 按 1 件处理；不得为了方便改写采集原值。普通单品 `subItems=[]` 时，严格核对按 `title + outerId + qty=1` 构造应退项。悦希 4 个固定历史例外虽然用条码型编码在档案V2「规格商家编码」字段定位，但查询成功后仍保留 ERP 返回的 `outerId`（`yx002`～`yx005`）作为退货入库核对编码；查询键和最终核对编码不得混用。具体白名单见 `docs/erp-query.md`。
