...
| 特殊悦希商品后再查普通商品时档案V2 `dataList 为空` | 上一轮「规格商家编码」残留，与本轮「主商家编码」叠加 | 查询前清空另一字段；固定例外和编码口径见 [erp-query.md](erp-query.md#查询字段选择与清场2026-09-10) |
...
| `waitFor 超时: 等待售后列表第2页刷新` | 固定清单处理期间，自动处理或客户取消使待处理列表缩页；平台底部总数还可能保持旧值，导致脚本等待已经不存在的第2页 | Step 14 只刷新当前售后列表一次，重新确认页面就绪、排序和稳定分页后从第1页定位当前工单；固定批次清单不重算。刷新后仍异常则停止，不循环刷新 |
| 已处理的反馈仍显示在“待洞察” | 手动修复规则后没有给 feedback 写入 `insightedAt` | 按下节精确归档对应反馈 ID，并读回验证可见待洞察数量 |
| 平台提醒出现“测试店铺” | 单元测试漏注入 `fetchAndCacheAlerts`，读取真实鲸灵页并污染生产缓存 | Step14 只调用注入依赖；缓存按 `accounts.json` 校验账号和店铺名，不匹配项自动失效；测试隔离见 [ops-testing.md](ops-testing.md#5-单元测试隔离) |
