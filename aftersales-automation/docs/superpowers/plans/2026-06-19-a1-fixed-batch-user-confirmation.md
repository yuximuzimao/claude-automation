...
- [x] 本轮进度需要持久化，但展示仍按原售后系统逻辑。允许在每个标签页增加店铺筛选项：全部、店铺 1、店铺 2……筛选不改变状态分类、不改变按时效排序。2026-06-27 用户追加要求：筛选后的店铺视角也必须成为批量操作作用域，不能出现“页面只看单店铺但后端仍全量批量重来/执行”的偏差；具体审查计划见 `docs/superpowers/plans/2026-06-27-live-tab-store-filter-and-legacy-cleanup.md`。
