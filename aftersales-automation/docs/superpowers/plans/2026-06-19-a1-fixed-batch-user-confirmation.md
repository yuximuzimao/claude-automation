...
5. 下一步：等待用户明确授权后重启加载并做 UI smoke test；按钮只调用后端 `POST /api/accounts/:num/a1-fixed-batch`，不传可篡改参数，不批量，不绕过状态/session 门禁。
6. 再下一步：自动执行真实工单必须另行处理 journal 恢复和人工审计路径。
7. 真机和正式入口仍需用户单独授权；不得从文档确认推导出更大范围授权。
