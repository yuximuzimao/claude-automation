...
- `lib/server/auto-execution-journal.js`：自动执行 intent / completed 防重复账本，已扩展状态机和 phase 门禁；恢复策略设计见 `docs/superpowers/plans/2026-06-27-auto-execution-journal-recovery-design.md`。
- `lib/server/auto-execution-recovery.js`：本地人工收口服务，负责同步更新 journal、queue、simulation/audit 和执行门禁；当前没有 CLI/API/UI 外部入口。

2026-06-26 已在用户授权下运行真实浏览器完成单工单采集验证；2026-06-26/27 已完成账号 14 关闭自动执行的最小整账号固定清单批次验证；2026-06-27 已接入并加固后端 `op-queue/API` 入口，且已接入前端单账号 no-auto 入队按钮代码。2026-06-27 已完成 auto-execution journal recovery 设计和 Phase 1 代码基础，明确人工归档必须同步更新 journal、queue、simulation/audit 和执行门禁。验证期间没有重启 server、没有真实 approve/reject。
...
以下风险仍不得扩大自动执行范围或接入真机。2026-06-27 已先完成第 4 项设计和 Phase 1 本地状态基础，但未实现 CLI/API/UI：

4. **自动执行锁 / intent 可能永久残留**：recovery 设计和 Phase 1 代码基础已完成，见 `docs/superpowers/plans/2026-06-27-auto-execution-journal-recovery-design.md`。核心要求：人工归档必须同步更新 journal、queue、simulation/audit 和执行门禁，不能只把 journal 标成 resolved；`manually_resolved` 是审计收口，不是自动重试放行。后续仍需实现 CLI/API/UI 恢复入口、入口确认门禁和端到端测试矩阵。
...
- 2026-06-27 auto-execution journal recovery 设计和 Phase 1 代码基础已完成：`docs/superpowers/plans/2026-06-27-auto-execution-journal-recovery-design.md`。该设计明确区分页动作不确定和本地写回失败；禁止盲重试 approve/reject；人工归档必须同步关闭 journal、queue、simulation/audit 和执行门禁。Phase 1 已实现状态机/phase/helper/本地 recovery service，但没有 CLI/API/UI 恢复入口。
- 2026-06-27 journal recovery Phase 1 审查修补后：`npm test` 242/242 通过。
- 测试通过只证明现有用例通过；账号 14 最小整账号批次只证明 no-auto 固定清单串行链路可跑通；后端队列入口、前端单账号 no-auto 按钮代码、live 店铺筛选代码、journal recovery Phase 1 本地基础已交付并加固；这些都不代表已重启加载、已有恢复 UI，或自动执行真实工单已可运行。
...
- 这个差异是正向推进，但范围不能外推：正式 UI 按钮代码虽已接入但未重启/只读冒烟，平台提醒收尾策略和自动执行真实工单仍未交付。
