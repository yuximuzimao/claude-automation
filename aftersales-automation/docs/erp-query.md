...
- `[∞/永久保留]` **#20 档案V2 数据源**：新版商品档案V2不再提供旧 `sv.dataList`；必须从当前可见 `.el-table.__vue__.store.states.data` 读取完整当前页数据，不能依赖 `.el-table__body tr` 的可见行数量
- `[∞/永久保留]` **#21 禁止空条件搜索**：搜索结果为空时，禁止发空条件搜索"验证页面"——会清空已有结果
- `[∞/永久保留]` **#22 档案V2 查询触发**：不要依赖模拟 Enter；新版页面在 `.search-wrap` Vue 组件上提供 `search()`，DOM 输入并派发 input/change 后直接调用该方法触发查询
...
- `[∞/永久保留]` **#33 档案V2 子商品数字定位**：新版已移除旧 `a.ml_15` class；先在当前可见主表按表头精确定位“子商品信息”列，再在目标行该列内找 `<a>`，并用 `innerText.trim()==预期数字` 精确匹配
- `[∞/永久保留]` **#42 店铺过滤器按账号切换**：商品对应表店铺过滤器必须按账号 note 查 `lib/erp/shop-map.js` 获取，禁止硬编码「杭州共途」
- `[∞/永久保留]` **#43 档案V2查询字段残留**：从悦希特殊「规格商家编码」查询切回普通「主商家编码」查询前，必须先清空规格字段；否则两个条件叠加会让正常商品误报表格结果为空。固定例外和最终核对编码见本页“查询字段选择与清场”。
