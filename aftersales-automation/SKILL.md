...
| `server.js` | Express 服务（port 3457），队列管理 + Web 面板 + 定时扫描调度。启动清理保留最新 500 条 simulation，同时通过 `lib/server/simulation-retention.js` 额外保护所有未归档工单引用的旧明细。定时扫描走 opQueue scan 并遵守 scanEnabled；legacy scan-all 不作为当前前端/A1处理入口 | 改 API/队列/定时任务/启动数据清理时 |
...
| `lib/server/data.js` | JSON/jsonl 数据持久化；统计累计处理量合并 live queue、归档 case 和现存 live simulation 并按工单号去重，评价正确率只按每单最新明确评价计算；反馈洞察用 `markFeedbackInsighted(ids)` 写 `insightedAt`，保留原文 | 改数据读写、统计口径或收口手动洞察时 |
...
| `public/app.js` | 前端主逻辑 — 7 个一级 Tab 渲染、“已自动执行”每页 10 条分页、统计累计口径与历史记录内嵌、快递行动分类 `isReturnWaitingAction()`、徽章计数、品牌分组、倒计时格式化 | 改前端展示/分类/统计逻辑时 |
...
6. **平台阶段观察**：步骤 10 为所有 48 小时工单保存列表原始 `商家-*` 阶段，步骤 14 写入 queue/simulation。默认只展示和复盘；当前唯一决策分支是“换货+商家-待商家二次发货”，观察期仍完整采集并保存原综合推理，只输出“无需处理、人工确认后手动归档”。执行按钮置灰，后端同时拒绝执行，不写平台备注、不自动归档。人工归档使用独立 `confirmed_no_action` 来源；同工单同阶段后续扫描只刷新观察时间，阶段变化才重新进入待确认；重新采集必须使用本次列表读取到的新阶段。
...
- **ERP 订单搜索错误只重搜一次**：ERP 存在无法仅靠 DOM 提前识别的隐性异常，因此定时扫描开始前无条件刷新一次；刷新后只验证登录、订单管理页、可见“系统单号”搜索框和 `mixKey` 已重新挂载。`erpSearch()` 保持原有 Enter 搜索，并逐行核验“平台交易号”包含本次子订单号；首次结果错误先强制刷新订单页再搜一次，总共最多 2 次。第二次仍失败立即报错，不继续物流采集、推理或整张工单流程。
...
- 前端：新增店铺通过 `POST /api/accounts/add` 创建账号并返回账号编号；重新登录通过 `POST /api/accounts/:num/relogin`。两者打开登录页后都进入同一 `reloginConfirm` 状态，必须同时提供「确认保存」和「取消」。
- 后端：两个入口统一通过 `lib/server/relogin-session.js` 启动临时登录进程并等待端口文件；`/relogin-confirm` 请求临时进程 `/confirm`，`/relogin-cancel` 请求 `/cancel` 并清理 `../sessions/.relogin-port-<num>`。
- 取消：点击后立即进入 `reloginCancelling`，禁用确认/取消并显示「取消中...」；只有后端成功返回才清理 `reloginConfirm`、恢复「重新登录」，失败则保留确认态供重试。不要用固定秒数判断，以按钮恢复为完成信号。
- 保存：首次新增且 Session 文件尚不存在时传 `--initialize-phone`，确认保存后从认证数据 `supplierInfo.supplierMobileList[0]` 初始化缺失的 `accounts.json.phone`；没有可用手机号不阻断 Session 保存。已有 Session 的普通重新登录不提取、不覆盖手机号，并继续用 `lib/jl-account-config.js` 保留 `phone/name/note/file`。
- 状态：确认保存成功后写 `ok`。历史或其他流程中的 `hasFile=true + status=unknown` 表示「已保存但未单账号验证」，UI 只显示「未扫描」、不显示重新登录；验证走安全打开账号或 A1 单账号流程，禁止恢复批量刷新状态。
...
| 12 | DOM 移除 Element UI 弹窗破坏 Vue 内部状态 | `el.parentNode.removeChild(el)` 移除 `.el-dialog__wrapper` 后 Vue 的 `dialogVisible` 仍为 true。下次点击子商品链接时 Vue 认为弹窗已打开，跳过打开逻辑 → "子商品弹窗未打开"。必须用 `btn.click()` 触发 Vue close 流程，并轮询等待弹窗从 DOM 消失。案例：2026-05-04 archive.js CLOSE_SUB_DIALOG_JS 用 DOM 移除 → 第二个工单起 subItems 全空 |
...
| 15 | ~~刷新状态/check-session 全链路~~ **已删除（2026-06-16 停旧系统）** | 原批量检测会短时连续登录多个账号。账号状态只允许通过店铺管理“打开后台”的安全编排或 A1 单账号流程确认；不要重建批量刷新状态功能。 |
...
| 23 | 登录确认态过早退出、没有退出路径，或首次新增未保存手机号 | 新增与重登必须共用 `reloginConfirm`；点取消后先进入 `reloginCancelling` 并锁住按钮，等 `/relogin-cancel` 成功后再清理确认态，失败则保留。登录页关闭或 port 文件不存在时也要退出确认态。首次新增且 Session 文件不存在时才允许从认证数据初始化缺失的手机号；普通重登必须保留旧 `phone`、不得覆盖。`unknown + hasFile` 表示未扫描验证，不是失效。 |
