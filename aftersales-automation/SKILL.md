...
| 35 | 拒绝取证后固定等 800ms 就判物流弹窗未关闭 | Element UI 关闭动画可能超过 800ms。拒绝流程必须复用 `logistics.js` 的局部关闭能力：主关闭按钮只点一次，按 300ms 只读轮询最多 5s；超时后才允许固定坐标后备一次。两种方式都失败时必须在打开拒绝表单前停止，禁止重试整个拒绝操作。2026-08-20 修复。 |
