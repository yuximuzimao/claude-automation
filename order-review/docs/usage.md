...
工具读取当前待审核页面序号1订单；本地保存方案不执行ERP。普通单包审核与多包拆分
均由用户逐单点击触发。多包拆分当前只运行到点击拆分二次确认，之后不核验结果、
不继续审核，并关闭自动刷新；用户需回到ERP人工核对。不能把自动刷新理解为自动
审核或拆分下一单。历史匹配和物流规则见
...
装箱实验是独立本地页面，不要求打开ERP，也不会连接CDP或执行审核。固定地址为
`http://127.0.0.1:3466/`，不会因为当前分支或 worktree 不同而自动换端口。建议始终从项目内固定启动器启动：

```bash
zsh run-packing-simulator.sh
```
...
只在同品牌普通候选纸箱中寻找一只可行单箱；手动指定后失败不能偷偷换箱。产品原箱也可出现在
手动下拉框，但只有当前实际占空间商品为单一品种、该原箱允许对应商品、数量落在已确认可发
范围且原箱尺寸已知时才显示；显示即表示可以作为指定容器进入几何模拟。缺尺寸原箱只在资料页
标“待补”，不会进入算法下拉框。自动模式当前不会拆多包，也不会用固定原箱规则提示答案。

包装资料只保留“包装纸箱 / 单品尺寸 / 包装规则”三个入口；包装纸箱和单品尺寸按KGOS/悦希
双列展示，待补状态跟随对应卡片，产品原箱作为纸箱标签。历史订单只读当前正式案例库并折叠
同一订单到最新有效版本；停用品牌不进入现役实验列表。加载历史订单只生成页面实验副本，
不修改 `cases.json` 或原人工包裹方案。

当前底层求解器已把箱内边界/碰撞与稳定性拆开：优先寻找完整底面支撑方案，预算内
找不到时可返回具有实际承重点的部分支撑方案，并显示最低支撑覆盖率。几何可放不等于
运输稳定；UNKNOWN也不能解释为装不下，支撑指标不自动形成新增业务规则。
