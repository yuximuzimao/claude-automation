...
- 该归档曾把 `splitResult.success=false` 当作失败，并因虚拟列表取消拆分后逐包明细核验；这两项结论于2026-08-05被后续方案取代。2026-09-17又进一步停用了拆分确认后的结果核验与审核，因此本阶段流程只作历史参考。
...
- 组合文档记录的是第一版限制；后续曾允许同一完整模块重复使用，但 2026-08-29
  已收紧为不得超过单个历史案例实际证明的最高次数。唯一且无冲突的精确组合仍可
  直接进入可编辑草稿，但仍不自动确认或操作 ERP；当前边界见本索引最后一节。
...
## 2026-08-14 纸箱尺寸与三维装箱底座

- `2026-08-14-carton-packing-foundation/neat-handoff.md`：尺寸目录、三维刚性长方体
  求解、保守内径规则、品牌与专用箱边界、历史包裹审计、线上规则优先级和
  `290 passed` 阶段收尾。
- 同目录保留原始数据采集清单、近似推荐方向和完整装箱底座说明，用于追溯本阶段
  的数据来源与设计取舍；当前续接只需先读 `../CURRENT.md`。

## 2026-08-27 拆分审单等待时序

- `2026-08-27-split-transition-wait-timing/neat-handoff.md`：混合拆分并审核四个
  关键节点等待时序细化为 `1s` → `300ms` → `600ms` → `1s`（新增点击主确定后
  `1s` 与核对通过后 `1s` 两段等待、展开结果行等待 `300ms` 改 `600ms`），
  `295 passed` 回归。

## 2026-08-28 拆分结果单向核验与两包实测

- `2026-08-28-split-single-pass-verification/neat-handoff.md`：移除结果核验阶段的
  两次回顶和第 1 条无意义视口定位，改为 `1→N` 单向读取；审核前以已核验的结果
  系统订单号检查当前挂载行，审核提交后才回到新的第 1 条，并确认全部结果订单
  已离开待审核列表。
- 完整自动化回归为 `296 passed`。用户已实际完成一笔较短的两包拆分并审核，确认
  没有多余上下滑动；三个及以上结果行和虚拟列表卸载场景仍待后续真实订单验证。
- 本阶段取代 2026-08-06 归档中的“逐包读取后先滚回第 1 行再审核”做法；旧文档
  保留当时事实，不再作为当前实现依据。
- 2026-09-17起，浮窗在点击拆分二次确认后立即停止，不再执行本阶段的单向结果核验或后续审核；恢复这些能力必须按当前TODO重新设计，不能直接复用本归档。

## 2026-08-29 历史包裹组合重复证据边界

- `2026-08-29-historical-composition-evidence-boundary/neat-handoff.md`：无花果果冻
  `40` 盒被错误拼成 `25 + 5 + 5 + 5` 的来源、正式案例与推荐事件证据、历史模块
  重复次数上限、界面“历史组合建议”语义和 `294 passed` 回归。
- 当前订单从未保存过完整四包方案；旧算法把只在单个历史方案中最多出现一次的
  `5` 盒尾包重复使用三次。本阶段改为同一模块不得超过单个历史案例实际证明的
  最高次数，没有证据时保持人工判断。
- 多个同时可行的两包方案如何比较、空间是否需要可视化仍未决定，统一留在
  `../CURRENT.md`，不得把本归档解释为已经决定开发 3D 功能。
