...
7. `果实进度` 的一行只是候选记录，不能因位于该 Sheet 就直接视为存在果实；必须同时核对 C 列来源和 D 列说明。D 列只描述获得精灵或精灵蛋时不生成 fruit，当前反例包括“唯一的迪莫”和“达到100研学绩点获得蛋”。
8. 同步与审计统一调用 `read-latest-excel.py` 的 `classify_fruit_row()`；遇到未确认的新来源必须报错并人工核对，禁止默认归入“限时活动”。
9. fruit 必须保存 Excel A 列的完整家族编号范围 `familyNumberRange`；页面展示该范围，不能只显示果实所挂最终形态的单个编号。
10. 果实名统一为“家族第一形态名 + 的果实”，不能使用 fruit 所挂最终形态名生成；例如魔力猫家族显示“喵喵的果实”、罗隐家族显示“阿米亚特的果实”。用户明确需要单独计数的形态专属果实写入 `additionalFruits`，例如“鸭吉吉-燃了鸭的果实”，不能与普通“鸭吉吉的果实”共用进度。
11. 已收集与当前能否获取必须分开记录。跨账号合并口径下，小号已有即保留已收集；渠道关闭时另写 `availability="渠道限定 · 当前不可获取"`，例如布瓜蝌的果实。
