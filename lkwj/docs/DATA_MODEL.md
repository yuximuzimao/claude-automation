...
`POST /api/save` 保存的是完整 `collections.json`，不是单字段补丁。为避免多个浏览器标签页互相覆盖，服务端对原始文件内容计算 `ETag`，并执行乐观并发控制：

1. 调用方先通过 `GET /api/data` 读取最新进度和响应头中的 `ETag`。
2. 保存时把该值放入 `If-Match`；缺少版本时服务端返回 `428`。
3. 如果文件已被其他页面更新，版本不一致时返回 `409`，旧快照不会落盘。
4. 保存成功后响应返回新的 `ETag`，调用方用它作为下一次保存版本。

主界面遇到 `428` 或 `409` 会重新加载服务器上的最新进度，并要求用户重新执行刚才的操作。其他脚本或页面必须遵守同一协议，不能直接发送无版本的整份快照，也不能为省事直接写 `collections.json`。
...
- 所有收集标签页按编号或各自稳定业务顺序连续展示，不分页；“状态=全部”时不拆成已收集/未收集分组，避免同一列表出现两套统计和视觉层级。
- 搜索或其他筛选生效时，统计统一使用“已收集 / 筛选”；“筛选”表示当前条件下的结果数量。
