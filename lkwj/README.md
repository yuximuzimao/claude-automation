...
`POST /api/save` 会整份保存 `collections.json`。页面或脚本必须先请求 `GET /api/data`，保留响应头中的 `ETag`，保存时再通过 `If-Match` 原样带回：

- 缺少 `If-Match` 时返回 `428`，表示调用方版本过旧，需要重新读取进度。
- `If-Match` 与当前文件不一致时返回 `409`，表示其他页面已经保存过，服务器会拒绝旧快照覆盖新进度。
- 保存成功后响应会返回新的 `ETag`，后续保存必须使用这个新值。

主界面遇到 `428` 或 `409` 会自动载入最新进度，并提示用户重新执行刚才的勾选。不要通过无条件写文件绕过这层保护。
...
| 状态 | 标签 / 数据 | 说明 |
|------|-------------|------|
| 已整理可用 | 精灵、课题、进化链、异色炫彩、多形态、精灵果实、家具、遗迹 | 最新 Excel 的 N.001–N.439 已完整同步；N.440“睡铃雪影娃娃”已建立名称占位。现有 440 只精灵、2192 条课题、191 条进化链、168 条家族果实记录、1 条形态专属果实记录和 144 个多形态项 |
