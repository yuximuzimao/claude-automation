...
const checks = [
  ...
  ['renderClothingSetMeta exists for set-level shared info', /function\s+renderClothingSetMeta\s*\(/.test(html)],
  ['renderClothingPieceRow exists', /function\s+renderClothingPieceRow\s*\(/.test(html)],
  ...
  ['clothing UI distinguishes set and single', html.includes('collectionType') && html.includes('套装') && html.includes('单件')],
  ['clothing UI shows set-level paired pet', html.includes('renderClothingSetMeta(group.set)') && html.includes('pairedPetName') && html.includes('配对精灵')],
  ['set piece rows are rendered without duplicated set method fields', html.includes("if (item.collectionType === 'set')") && html.indexOf("if (item.collectionType === 'set')") < html.indexOf('const source = item.collectionType')],
