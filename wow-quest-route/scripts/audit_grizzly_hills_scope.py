...
AUTO = ROOT / "data/route-atlas/grizzly-hills-candidate-summary.json"
OUT = ROOT / "data/route-atlas/grizzly-hills-scope-audit.json"
REPORT = ROOT / "docs/analysis/2026-08-18-grizzly-hills-scope-audit.md"
ZONE_ID = 394

# Current from-zero Horde axis and outdoor policy. These are structural, not speed/economic pruning.
STRUCTURAL_EXCLUDE = {
    11981: "mutually_exclusive_with_12074_current_axis",
    11997: "alliance_westfall_brigade_breadcrumb",
    12434: "repeatable_zero_xp_second_round_not_generated",
    12446: "alliance_counterpart",
    12763: "mutually_exclusive_with_carried_12789",
}
...
def reason(task: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if task.get("assigned_zone_id") != ZONE_ID and ZONE_ID not in (task.get("touching_northrend_zone_ids") or []):
        return "boundary_only", ["does_not_touch_grizzly"]
    ...
    if task.get("is_repeatable") or task.get("is_daily") or task.get("is_weekly") or task.get("is_monthly"):
        # First round repeatables can be evaluated separately only if they carry one-time-like XP value;
        # current leveling baseline does not automatically generate repeat rounds.
...
def main() -> None:
    ...
    # Formal zone ownership uses the quest's assigned parent zone. Merely touching Grizzly through a
    # generic item-source expansion is not enough (it previously pulled in Howling Fjord quests).
    ...
    # Promote zero-XP mandatory prerequisites that are in the Grizzly universe and structurally valid.
    changed = True
    while changed:
        changed = False
        by_id = {row["quest_id"]: row for row in rows}
        for qid in sorted(include_ids):
            task = next((task for task in tasks if int(task["quest_id"]) == qid), None)
            if not task:
                continue
    ...
    lines = [
        ...
        f"- Questie assigned到灰熊丘陵的诺森德任务卡：{len(rows)}。",
        f"- 正式可执行候选：{len(formal_ids)}。",
        f"- 状态统计：`{dict(sorted(counts.items()))}`。",
        f"- requiredLevel分布：`{dict(sorted(level_counts.items(), key=lambda x: (x[0] is None, x[0] or 0)))}`。",
        f"- 旧自动路线选中：{len(auto_ids)}；正式池相对旧自动路线新增{len(old_auto_missing)}，旧自动路线多出{len(old_auto_extra)}。",
        "",
        "## 旧自动路线漏召回的正式候选",
        "",
    ]
