...
OUT = ROOT / "data/route-atlas/grizzly-hills-scope.json"
REPORT = ROOT / "docs/analysis/2026-08-18-grizzly-hills-scope.md"
ZONE_ID = 394
...
def main() -> None:
    ...
    lines = [
        "# 灰熊丘陵正式scope（全清基线）",
        "",
        "- 目标：建立连续户外全清任务池；只做结构性排除，不做速度/金币裁剪。",
    ...
    for qid in old_auto_extra:
        ...
        if row:
            ...
            lines.append(f"- {qid}：未出现在灰熊assigned任务卡中")
    lines.extend(["", "## 正式候选", ""])
