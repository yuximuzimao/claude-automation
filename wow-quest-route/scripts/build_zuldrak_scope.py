...
OUT = ROOT / "data/route-atlas/zuldrak-scope.json"
REPORT = ROOT / "docs/analysis/2026-08-19-zuldrak-scope.md"
ZONE_ID = 66
...
def main() -> None:
    ...
    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"assigned": len(rows), "formal": len(formal_ids), "missing_from_old_auto": len(old_auto_missing), "old_auto_extra": len(old_auto_extra), "status_counts": dict(sorted(counts.items())), "out": str(OUT.relative_to(ROOT))}, ensure_ascii=False, indent=2))
