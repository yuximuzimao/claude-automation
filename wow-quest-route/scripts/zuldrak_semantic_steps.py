...
def apply_zuldrak_semantic_overrides(points: list[list[Any]], groups: list[dict[str, Any]]) -> None:
    specs: dict[int, dict[str, Any]] = {
        ...
        4: {
            ...
            "note_html": notes_html(note_block("黑暗的地平线", "与高尔巴格交谈开始飞行侦察；飞行结束后回达库鲁交任务，此时《潜入沃尔塔鲁斯》完成。保留斯特凡号角，传送回痛苦之匣后直接使用。"), note_block("官大一级压死人", "控制荒芜恶鬼采集荒芜水晶，不是普通地面拾取。" + status_span("五开待实测") + "确认恶鬼采集是否共享。"), note_block("目前为止，一切很糟", status_span("五开待实测") + "确认瘟疫锅使用是否共享。"), note_block("背叛", status_span("五开待实测") + "确认战斗和最后愿望是否需要五号分别执行；最后愿望只有短时间可用，击败达库鲁后立刻使用。")),
