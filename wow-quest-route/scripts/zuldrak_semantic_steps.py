    ...
    specs: dict[int, dict[str, Any]] = {
        ...
        2: {
            ...
            "points": [
                ...
                {"title": "北伐军战士达加斯", "action": "推进《朋友的意义……》\n沿路推进《从无到有》"},
                {"title": "戈尔克", "action": "推进《朋友的意义……》\n戈尔克 → 接《圣光不能为我复仇》\n↳ 做《圣光不能为我复仇》\n戈尔克 → 交《圣光不能为我复仇》"},
                {"title": "布尔", "action": "推进《朋友的意义……》"},
                {"title": "盖米尔囚笼", "action": "盖米尔 → 接《风暴将至》"},
                {"title": "北伐军前线营地", "action": "工程师里德 → 交《从无到有》《风暴将至》 → 接《拯救盖米尔》\n北伐军战士马克拉尔 → 交《朋友的意义……》"},
                {"title": "北伐军前线周边", "action": "沿路推进《拯救盖米尔》", "note": "《拯救盖米尔》：先在前线周边收女妖精华；硅藻土不在这里，继续去黑锋哨站入口约(13.3,74.8)补齐。", "fivebox": "确认女妖精华与硅藻土是否个人掉落。"},
            ],
            "action_html": [point_anchor("北伐军前线营地"), npc_actions("北伐军战士马克拉尔", turns=("北伐军前线营地",), accepts=("朋友的意义……",)), npc_actions("工程师里德", accepts=("从无到有",)), task_text_line("找到达加斯，推进 ", "朋友的意义……"), task_text_line("沿路推进 ", "从无到有"), point_anchor("戈尔克"), task_text_line("找到戈尔克，推进 ", "朋友的意义……"), npc_actions("戈尔克", accepts=("圣光不能为我复仇",)), do_line("圣光不能为我复仇"), npc_actions("戈尔克", turns=("圣光不能为我复仇",)), task_text_line("找到布尔，完成 ", "朋友的意义……"), npc_actions("盖米尔", accepts=("风暴将至",)), point_anchor("北伐军前线营地"), npc_actions("工程师里德", turns=("从无到有", "风暴将至"), accepts=("拯救盖米尔",)), npc_actions("北伐军战士马克拉尔", turns=("朋友的意义……",)), task_text_line("沿路推进 ", "拯救盖米尔")],
        ...
        3: {
            ...
            "points": [
                ...
                {"title": "黑锋哨站", "action": "确认《潜入沃尔塔鲁斯》仍活动\n使用迷惑项圈准备进入沃尔塔鲁斯"},
            ],
            "action_html": [do_at("黑锋哨站入口", "拯救盖米尔"), point_anchor("黑锋哨站"), npc_actions("斯特凡·瓦杜", turns=("黑锋哨站",), accepts=("纳斯和巨魔的毛发",)), system_line("开飞行点：黑锋哨站（五号分别）", "ra-flightpoint"), do_at("干瘪巨魔", "纳斯和巨魔的毛发"), item_accept_line("某种邀请……"), point_anchor("黑锋哨站"), npc_actions("斯特凡·瓦杜", turns=("某种邀请……",), accepts=("幸免于难",)), task_text_line("保持已完成未交：", "纳斯和巨魔的毛发"), npc_do_line("血玫瑰达图拉", "幸免于难"), npc_actions("斯特凡·瓦杜", turns=("幸免于难",), accepts=("无处可藏",)), do_at("痛苦之匣下层", "无处可藏"), npc_actions("斯特凡·瓦杜", turns=("无处可藏",), accepts=("乔装打扮",)), do_at("骨肠", "乔装打扮"), npc_actions("骨肠", accepts=("喂饱食尸鬼",)), do_line("喂饱食尸鬼"), npc_actions("骨肠", turns=("喂饱食尸鬼",)), point_anchor("黑锋哨站"), npc_actions("斯特凡·瓦杜", turns=("乔装打扮", "纳斯和巨魔的毛发"), accepts=("潜入沃尔塔鲁斯",)), npc_actions("血玫瑰达图拉", accepts=("银色前沿",)), point_anchor("北伐军前线营地"), npc_actions("工程师里德", turns=("拯救盖米尔",), accepts=("唯一的希望",)), do_at("盖米尔囚笼", "唯一的希望"), npc_actions("盖米尔", turns=("唯一的希望",), accepts=("风暴之王的复仇",)), do_at("盖米尔载具", "风暴之王的复仇"), npc_actions("北伐军战士马克拉尔", turns=("风暴之王的复仇",)), point_anchor("黑锋哨站"), task_text_line("确认仍活动：", "潜入沃尔塔鲁斯"), raw_line("使用迷惑项圈准备进入沃尔塔鲁斯")],
        ...
        6: {
            ...
            "points": [
                {"title": "达克迦尔岗哨", "action": "格隆迪尔上尉 → 接《温暖的篝火》\n沿路推进《温暖的篝火》", "note": "《温暖的篝火》：枯死荆木沿路固定拾取；若不够，后续返回达克迦尔前再补。"},
                {"title": "达克索塔", "action": "布兰顿上尉 → 接《止痛药》\n鲁伯特上尉 → 接《扔手雷》\n罗杰斯博士 → 接《一个也不能少》"},
                {"title": "水罂粟 / 水元素走廊", "action": "↳ 做《止痛药》《达卡莱巨魔不需要水元素！》《珍贵的元素液体》"},
                {"title": "蛛魔弹坑", "action": "↳ 做《扔手雷》\n《一个也不能少》暂不做", "note": "《一个也不能少》：这里先不救人；先回达克索塔交《扔手雷》并接出《茧中人》，再把两个救援任务合并完成。"},
            ...
            "action_html": [point_anchor("达克迦尔岗哨"), npc_actions("格隆迪尔上尉", accepts=("温暖的篝火",)), task_text_line("沿路推进 ", "温暖的篝火"), point_anchor("达克索塔"), npc_actions("布兰顿上尉", accepts=("止痛药",)), npc_actions("鲁伯特上尉", accepts=("扔手雷",)), npc_actions("罗杰斯博士", accepts=("一个也不能少",)), do_at("水罂粟 / 水元素走廊", "止痛药", "达卡莱巨魔不需要水元素！", "珍贵的元素液体"), do_at("蛛魔弹坑", "扔手雷"), task_text_line("暂不做：", "一个也不能少"), point_anchor("达克索塔"), npc_actions("布兰顿上尉", turns=("止痛药",)), npc_actions("鲁伯特上尉", turns=("扔手雷",), accepts=("茧中人",)), npc_actions("穆尔沙·月影中士", accepts=("通灵师之死",)), npc_actions("专家考格维尔", accepts=("撒网者的喷丝头",)), do_at("蛛魔救援区", "一个也不能少", "茧中人"), do_at("哈沙尔", "通灵师之死", "撒网者的喷丝头"), point_anchor("达克索塔"), npc_actions("罗杰斯博士", turns=("一个也不能少",)), npc_actions("鲁伯特上尉", turns=("茧中人",), accepts=("纯粹的邪恶",)), npc_actions("穆尔沙·月影中士", turns=("通灵师之死",), accepts=("腐蚀者玛拉斯",)), npc_actions("专家考格维尔", turns=("撒网者的喷丝头",), accepts=("坠毁的喷射器",)), do_at("达克索塔北侧", "腐蚀者玛拉斯", "坠毁的喷射器", "纯粹的邪恶"), point_anchor("达克索塔"), npc_actions("穆尔沙·月影中士", turns=("腐蚀者玛拉斯",)), npc_actions("专家考格维尔", turns=("坠毁的喷射器",), accepts=("缠绕投网器",)), do_at("药剂喷射器", "缠绕投网器"), npc_actions("专家考格维尔", turns=("缠绕投网器",)), object_actions("希姆埃巴雕像", turns=("希姆埃巴的祝福",)), point_anchor("银色前沿"), npc_actions("亚克斯中尉", turns=("达卡莱巨魔不需要水元素！",)), npc_actions("妖术师乌布戈", turns=("珍贵的元素液体",), accepts=("蘑菇混合剂",)), npc_actions("伊崔格", turns=("纯粹的邪恶",)), npc_actions("学徒匹斯波特", accepts=("贪吃的蜥蜴",)), do_at("银色前沿东北", "蘑菇混合剂", "贪吃的蜥蜴"), task_text_line("沿路补 ", "温暖的篝火"), npc_actions("格隆迪尔上尉", turns=("温暖的篝火",)), point_anchor("银色前沿"), npc_actions("妖术师乌布戈", turns=("蘑菇混合剂",), accepts=("过犹不及",)), npc_actions("炼金师菲肯斯坦", turns=("贪吃的蜥蜴",)), do_at("西莱图斯先知", "过犹不及"), point_anchor("银色前沿"), npc_actions("妖术师乌布戈", turns=("过犹不及",)), npc_actions("指挥官库恩斯", turns=("巡逻任务",), accepts=("巫医库弗",)), point_anchor("达拉然短往返"), system_line("博学者泰罗努斯三世 → 传送到达拉然"), npc_actions("狡猾的维克斯", accepts=("勇士的召唤！",)), system_line("若未开达拉然飞行点则开启", "ra-flightpoint"), system_line("系统飞行：达拉然 → 银色前沿", "ra-flightpath")],
        ...
        9: {
            ...
            "points": [
                ...
                {"title": "奎丝鲁恩三名高阶祭司", "action": "↳ 做《地狱的复仇》\n奎丝鲁恩之魂 → 交《地狱的复仇》 → 接《最后一步》\n沿路推进《拎尾巴》", "note": "《地狱的复仇》：三名高阶祭司都要先使用奎丝鲁恩妖术棒，再击杀。", "fivebox": "确认妖术棒与拖带目标是否逐号操作。"},
            ...
            "action_html": [npc_actions("奎丝鲁恩之魂", turns=("寻找风蛇女神",), accepts=("创造条件",)), do_at("奎丝鲁恩", "创造条件"), npc_actions("奎丝鲁恩之魂", turns=("创造条件",), accepts=("复仇的基础",)), do_at("奎丝鲁恩", "复仇的基础"), npc_actions("奎丝鲁恩之魂", turns=("复仇的基础",), accepts=("地狱的复仇",)), do_at("奎丝鲁恩三名高阶祭司", "地狱的复仇"), npc_actions("奎丝鲁恩之魂", turns=("地狱的复仇",), accepts=("最后一步",)), task_text_line("沿路推进 ", "拎尾巴"), point_anchor("哈克娅祭坛"), npc_actions("哈克娅", turns=("最后一步",), accepts=("死去神灵的血液",)), do_at("犸托斯祭坛", "死去神灵的血液"), point_anchor("哈克娅祭坛"), npc_actions("哈克娅", turns=("死去神灵的血液",), accepts=("种瓜得瓜种豆得豆",)), do_at("虚弱的奎丝鲁恩先知", "种瓜得瓜种豆得豆"), system_line("使用炉石：希姆托加", "ra-hearth")],
        ...
        11: {
            ...
            "points": [
                {"title": "希姆鲁克守卫者", "action": "推进《诸神的指引》", "note": "《诸神的指引》：先从希姆鲁克守卫者取得守卫者精华；两份精华均按个人掉落处理，以五号最慢角色为准。"},
            ...
            "action_html": [task_text_line("推进 ", "诸神的指引"), do_at("奎丝鲁恩祭坛典狱官", "诸神的指引"), npc_actions("哈克娅", turns=("诸神的指引",), accepts=("在佐尔赫布相会",)), do_at("佐尔赫布召唤圈", "在佐尔赫布相会"), point_anchor("古达克飞行点"), system_line("系统飞行：古达克 → 希姆托加", "ra-flightpath"), npc_actions("哈克娅", turns=("在佐尔赫布相会",), accepts=("未完的事情",))],
        ...
        12: {
            ...
            "summary": "系统飞到古达克；杜布拉金分别向托玛尔、拜基妮交两条携带任务，哈瓦纳接迅猛龙卵；五号完成后回哈瓦纳交付，祖达克收口。",
            "points": [
