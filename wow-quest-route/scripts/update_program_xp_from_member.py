...
from lib.route_xp import update_route_program_xp_from_profile
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Recompute Program XP from one changed member through the dependent suffix. "
            "All earlier member XP reports are preserved exactly."
        )
    )
    ...
    upper = (
        runtime["external_xp_upper_bound_by_profile"]
        if "external_xp_upper_bound_by_profile" in runtime
        else current.get("external_xp_upper_bound_by_profile")
    )
    payload = update_route_program_xp_from_profile(
        program,
        profiles,
        ...
        external_xp_upper_bound_by_profile=upper,
    )
