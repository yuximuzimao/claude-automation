    ...
    specs: dict[int, dict[str, Any]] = {
        1: {
            ...
            "points": [
                ...
                {"title": "沃德伦", "action": "↳ 做《沃德伦的领主》", "note": "《沃德伦的领主》：共享：骑任务龙完成载具战；贴近目标，高伤技能冷却好就用。"},
            ...
            "note_html": notes_html(note_block("赚外快 / 灰狼的毛皮 / 缚焰者的秘密", status_span("不共享") + "任务物为个人掉落，五号分别拾取。"), note_block("显示力量", status_span("共享") + "击杀进度五号同步。"), note_block("寻找溶解剂", status_span("五开待实测") + "Element 115 是否能同一刷新点连续五号拾取、返程窗口是否分别计时。拾取后立即原路返回古图尔。"), note_block("沃德伦的领主", status_span("共享") + "骑任务龙完成载具战；贴近目标，高伤技能冷却好就用。")),
        ...
        3: {
            ...
            "note_html": notes_html(note_block("抓巨魔", status_span("共享")), note_block("停战？", status_span("共享") + "任务刀就在花岗岩之泉附近树桩；使用任务刀完成动作后再离开。"), note_block("幻象之瓶", status_span("不共享") + "水草叶、3片朦胧叶和水晶瓶都齐再离开；水晶瓶在花岗岩之泉商人购买，一次购买直接得到5个，只需买一次。"), note_block("清理天灾", status_span("共享") + "木乃伊就在花岗岩之泉营地附近。"), note_block("蘑菇汤！", status_span("不共享") + "蛇眼、雪帽和甜根在北侧区域采集。"), note_block("解读象形文字", status_span("不共享") + "冰冻魔精可交易；每号都要自己在火盆使用药剂召唤达库鲁影像，召唤NPC不共享。当前号背包若还有超过任务所需的多余冰冻魔精会无法正常交任务，交前先移走多余魔精。"), note_block("必要的牺牲", status_span("不共享") + "所需魔精由五号依次拾取，预言者之眼也要五号依次交互取得，再分别到火盆完成交付。")),
        ...
        4: {
            ...
            "note_html": notes_html(note_block("征服斗兽场", status_span("五开待实测") + "确认五号同时有任务时是否一场战斗同步完成。前四轮交赌徒维尔金，下一轮由格里尼克斯接；最终《摊牌》交高戈娜。"), note_block("白肩鹰的眼睛", "与沃达希尔三洞同片完成；三处洞口及中部山地周围白肩鹰密集，直接对6只白肩鹰使用银色羽毛。")),
