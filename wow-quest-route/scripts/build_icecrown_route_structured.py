...
PRIMARY_STEP_ANCHOR: dict[int, tuple[float, float]] = {
    1: ANCHORS["银色前线基地"], 2: ANCHORS["银色前线基地"], 3: ANCHORS["回音谷"],
    4: ANCHORS["回音谷"], 5: ANCHORS["银色前线基地"], 6: ANCHORS["奥格瑞姆之锤"],
    7: ANCHORS["暗影拱顶"], 8: ANCHORS["暗影拱顶"], 9: ANCHORS["Savage Ledge"],
    10: ANCHORS["白骨女巫"], 11: ANCHORS["Jotunheim"], 12: ANCHORS["Jotunheim"],
    13: ANCHORS["白骨女巫"], 14: ANCHORS["瓦哈拉斯"], 15: ANCHORS["死亡高地"],
    16: ANCHORS["先锋军港口"], 17: ANCHORS["先锋军港口"], 18: ANCHORS["黑色观察站"],
    19: (58.3, 46.0), 20: ANCHORS["破碎前线"], 21: ANCHORS["冰冠堡垒"],
    22: ANCHORS["莫德雷萨"], 23: ANCHORS["莫德雷萨"], 24: ANCHORS["辛达苟萨之墓"],
    25: ANCHORS["黑色观察站"], 26: ANCHORS["北伐军之峰"], 27: ANCHORS["沉默墓地"],
    28: ANCHORS["银色前线基地"], 29: ANCHORS["北伐军之峰"], 30: ANCHORS["北伐军之峰"],
    31: ANCHORS["荒凉之门"], 32: (53.7, 45.6), 33: ANCHORS["奥尔杜萨"],
    34: ANCHORS["荒凉之门"], 35: ANCHORS["暗影拱顶"], 36: ANCHORS["哭泣采掘场"],
    37: ANCHORS["恐惧之门"], 38: ANCHORS["科雷萨"], 39: ANCHORS["玛雷卡里斯"],
    40: ANCHORS["苦难高地"],
}
...
def note_html(step: dict) -> str:
    ...
    for card in (step.get("task_cards") or {}).values():
        ...
        if fivebox:
            label = "五开待实测：" if "待实测" in fivebox else "五开："
            body += f'<div class="ra-fivebox-line"><span class="ra-pending">{label}</span>{html.escape(fivebox)}</div>'
...
def main() -> None:
    ...
    for step in draft.get("steps", []):
        ...
        groups.append({
            ...
            "summary": str(step.get("exit") or step.get("gate") or "按本段动作顺序完成后继续下一段。"),
    ...
    route = {
        ...
        "status": "structured_candidate_pre_geometry_cold_read",
        "title": "冰冠冰川 · 80级五开整图路线",
        "sub": "银色前线基地起步，依赖解锁与任务链按地理节点插入；移动飞艇只在自然回访节点处理，布雷登布莱德跨大陆链在北伐军之峰节点插入后继续向西清图。",
        "badge": f"炉石：格罗玛什坠毁点 → 暗影拱顶\\n预计总时间：{policy.get('route_total_center_minutes', 0)/60:.1f}小时（首跑前预算）",
        "image": "maps/210-icecrown-hd.jpg",
        "legend": "",
        "footer": "23项五开机制保留首跑核对；路线本体不因此生成条件分支。",
        "labels": [[x, y, name] for name, (x, y) in ANCHORS.items() if name not in {"奥格瑞姆之锤"}],
        "points": points,
        "defaultIndex": 0,
        "phaseColors": {f"ice{i:02d}": "#94a3b8" for i in range(1, 41)},
        "displayName": "冰冠冰川",
        "stepGroups": groups,
        "defaultGroupIndex": 0,
        ...
        "timing": {
            ...
            "actualRuns": [],
            "model": "icecrown_40_step_marginal_pre_live_v1",
        },
        "geometryAudit": {
