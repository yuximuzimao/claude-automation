...
- 项目唯一长期目标是把“任务路线”作为独立打金方式做成可长期复用的高净金币/真实墙钟系统；升级、80级后一次性任务清理、路线设计和页面工具都服务于这一目标，不在本文件固定某一批角色的等级终点。
- **具体当前等级、地图、任务状态、当前地图轴和阶段策略不得复制到本文件长期维护**，统一以 `docs/verified-routes/CURRENT.md` 为唯一当前真值。
- 路线方法：当前任务中心尽量闭环、最少重复横穿、自然衔接下一任务中心/地图；未满级阶段不能为兑现外部口述等级在已清空地图无任务刷怪，满级阶段则按任务收益与真实墙钟继续清理一次性任务。
- 用户实跑反馈优先于桌面模型；确认后先修当前执行稿，再写observations/任务卡，跨地图长期有效的方法才进入 `docs/rules/`。
- 其它职业/路线母版可作为后续资料，但是否启用属于CURRENT，不在稳定入口里写死。
- 接不到、进不去、交不了、服务器位面异常的任务记录到 `data/observations/blocked-tasks.json` 并按当前路线目标绕过或重算，不能让单个阻断任务卡死整条执行路线。
