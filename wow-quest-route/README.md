...
项目已经从“首组血精灵圣骑士1—80快速升级”调整为两阶段循环：

1. 首组五个血精灵圣骑士优先稳定升到55级，用于解锁后续死亡骑士创建条件。现有`data/routes/simple-leveling-route.html`完整保留，作为首组实跑参考和风险记录，不再把“一小时20级”当作这套单主控五开的验收标准。
2. 后续每轮建立五个55级血精灵死亡骑士，完整跑55—80级主要打金地图。第一轮优先覆盖全部可执行任务，记录阻断、死亡点、洞穴密度、回头路和逐号操作；重复实跑后再冻结为稳定打金流程。

当前主页面为`data/routes/dk-55-80-world-tasks.html`：覆盖死亡骑士55—80级全世界户外任务，按地图展示接取、目标和交付步骤。它是完整任务母版，不代表所有道路和建筑层级已经实跑证明最优。
...
- `data/routes/dk-55-80-world-tasks.html`是当前主母版，覆盖死亡骑士出生链及55—80级可执行户外任务，并剔除联盟不可交付区域；当前生成结果为42张地图、1858个任务、2855个页面步骤。
- 死亡骑士母版优先保证完整覆盖，全部任务均进入地图流程，不再以“经验足够就跳图”为目标；第一轮实跑后再按打金收益、死亡风险和耗时划分固定主图、次图与跳过任务。
- `data/routes/simple-leveling-route.html`继续保留为首组圣骑士1—55参考。逐日岛和永歌森林前段有实跑修订，幽魂之地与贫瘠之地仍属于验证版，后续占位不再阻塞死亡骑士主线建设。
