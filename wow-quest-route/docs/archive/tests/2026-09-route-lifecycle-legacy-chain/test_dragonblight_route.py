...
PROFILE = ROOT / "data/route-profiles/dragonblight-fivebox.json"
...
def test_dragonblight_route_uses_transport_and_player_facing_narf_fix():
    ...
    narf_accepts = [
        ...
        if action.get("kind") == "accept" and action.get("task_id") == 12043
    ]
    assert len(narf_accepts) == 1
    assert narf_accepts[0]["npc_name"] == "纳尔弗"
