...
# The first legacy action line contains cross-map movement plus three task actions.  Its leading
# "奥格瑞玛" token is a location, not an NPC.  Keep the previously audited task-specific actors
# explicit instead of letting the legacy free-text parser regress them on rerun.
NPC_OVERRIDES = {
    (9407, "accept"): "督军达图恩",
    (9407, "turnin"): "沃雷恩中将",
    (10120, "accept"): "沃雷恩中将",
...
def _task_name_index(cards: dict[int, dict[str, Any]], needed_names: set[str]) -> dict[str, int]:
    result: dict[str, int] = {}
    ...
    for task_id, card in cards.items():
        ...
        if name in result:
            ...
            result[name] = task_id
    if duplicates:
        raise RuntimeError(f"duplicate Task Card names used by this route require explicit migration identity: {duplicates}")
    missing = sorted(needed_names - set(result))
    if missing:
        raise RuntimeError(f"route task names without Task Cards: {missing}")
    return result
...
def _migrate_actions(
    ...
    for point_index, point in enumerate(route.get("points", [])):
        ...
        # Preserve the two cross-map movement atoms that were embedded in legacy prose instead of
        # treating them as disposable display text.
        if point_index == 0:
            add(
                {
                    "kind": "fixed_transport",
                    "location_ref": point_ref,
                    "from_name": "奥格瑞玛·精神谷传送门区",
                    "to_name": "诅咒之地·黑暗之门前",
                },
                point_index,
            )
        if title == "塞纳里奥哨站 → 赞加沼泽":
            add(
                {
                    "kind": "move",
                    "location_ref": point_ref,
                    "from_name": "塞纳里奥哨站",
                    "to_name": "赞加沼泽·塞纳里奥庇护所",
                },
                point_index,
            )
        ...
        for raw_line in action_text.splitlines():
            ...
            for match in matches:
                ...
                action_npc = NPC_OVERRIDES.get((task_id, kind), npc)
                if action_npc and kind in {"accept", "turnin"}:
                    action["npc_name"] = action_npc

                if task_id == 9407 and kind == "turnin":
                    add(
                        {
                            "kind": "fixed_transport",
                            "location_ref": point_ref,
...
def build_profile() -> dict[str, Any]:
    ...
    needed_names = {
        ...
        for name in TASK_RE.findall(str(point[3] if len(point) > 3 else ""))
    }
    task_name_to_id = _task_name_index(cards, needed_names)
...
def main() -> None:
    raise SystemExit('RETIRED: one-time Route Profile v1 migration is preserved for history only; formal Profile v2 is now maintained through the lifecycle truth source.')
