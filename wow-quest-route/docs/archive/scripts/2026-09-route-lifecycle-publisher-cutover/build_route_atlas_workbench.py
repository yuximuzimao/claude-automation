...
(function installRouteAtlasSemanticPrototype(){
  if(!document.getElementById('raSemanticPrototypeStyle')){
    ...
    style.textContent=`
      ...
      .ra-sequential-loot{font-weight:700;color:#d8c8ff}
      .ra-special{font-weight:700;color:#ffd66b}
...
def main() -> None:
    raise RuntimeError(
        "RETIRED: the legacy workbench-routes.json HTML builder cannot write the formal Route Atlas; "
        "use scripts/rebuild_route_profile.py --all --write-assets"
    )
