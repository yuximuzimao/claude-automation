...
状态：2026-08-15基础层历史审计；**只保留候选/机制事实，不承担当前任务删留目标。当前唯一目标与经济取舍规则以`docs/rules/README.md`和`leveling-and-selection.md`为准。**
...
- 北风主候选Scope分布：`{"defer_future_level_revisit": 5, "exclude_alliance_or_other_faction": 81, "exclude_deprecated_or_system": 6, "exclude_dungeon_or_raid": 4, "exclude_no_xp": 2, "exclude_profession": 5, "exclude_repeatable_calendar": 4, "exclude_server_variant": 3, "include_later_cross_map_followup": 1, "include_later_cross_map_inbound": 1, "include_leveling_cross_map_outbound": 2, "include_leveling_local": 160, "include_structural_zero_xp_prerequisite": 1}`。
- 边界引用Scope分布：`{"boundary_followup_reference": 5, "boundary_irrelevant_reference": 5, "boundary_prerequisite_reference": 2}`。
- Target Cluster：158个，其中多任务共享实体簇22个；普通目标为空的纳入任务72个。
- Questie extraObjectives：纳入任务共30条特殊机制事实；形成37个实体锚点簇和4个纯坐标锚点。
- 候选池可错过/互斥约束：2条。
...
- PASS `questie_extra_objectives_materialized`：{"included_extra_objectives": 30, "entity_anchor_clusters": 37, "coordinate_anchors": 4}
- PASS `included_tasks_have_viable_external_prerequisites`：[]
- PASS `video_reference_groups_resolved_or_intentionally_excluded`：[]
...
- 若路线计划完成`11574`《危在旦夕》，则必须先接，再让`11587`《越狱》进入任务栏或完成；否则Questie `exclusiveTo`会使前者失去可接资格。是否值得保留该可错过任务由经济边际比较决定。
- 若路线计划完成`11591`《钢腭的车队》，则必须先接，再让`11592`《攻击！》、`11593`《亡者的尊严》、`11594`《让他们安息》进入任务栏或完成；否则Questie `exclusiveTo`会使前者失去可接资格。当前基线路线不做11591。
- 初次北风自然出图：`11930`《横贯冰原》、`12117`《前往莫亚基港口》；两者接取后会占用任务栏，直到龙骨荒野对应交付点释放。
- 后续回北风链：`13242`《黑暗的骚动》需要先完成龙骨荒野`12500`《返回安加萨》，之后回战歌要塞交；再解锁`13257`《战争的使者》并前往奥格瑞玛。这两条保留在完整知识候选宇宙，但是否回访由后续经济边际比较决定。
- 一跳跨图边界引用：
...
- `11905`《延缓灾难》 → `exclude_dungeon_or_raid`：current_fivebox_outdoor_leveling_route_does_not_run_instances
- `11911`《苏醒》 → `exclude_dungeon_or_raid`：current_fivebox_outdoor_leveling_route_does_not_run_instances
- `11973`《战争的囚徒》 → `exclude_dungeon_or_raid`：current_fivebox_outdoor_leveling_route_does_not_run_instances
- `13095`《他们丝毫不感到羞愧吗？》 → `exclude_dungeon_or_raid`：current_fivebox_outdoor_leveling_route_does_not_run_instances
