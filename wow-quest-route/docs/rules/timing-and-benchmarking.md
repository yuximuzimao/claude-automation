...
固定10任务日常是独立打金方式，不与DK一次性任务路线G/小时混算。2026-09-25用户确认最近五轮规范执行墙钟为32、33、32、33、35分钟；从现在起正式预计时间固定为**33分钟/轮**。以后用户只说“日常任务”“固定10任务日常”或需要用日常时间参与计算时，默认直接使用33分钟，不再用早期45分40秒、39分22秒等历史样本重新滚动平均；只有用户明确改变任务集合或执行路线后才重新校准。

这33分钟的基准范围固定为：**银白比武场炉石点开始 → 接《实战训练》 → 飞艇接其余日常 → 完成两块任务环线 → 飞船交任务 → 炉石返回银白比武场 → 交《实战训练》结束**。进入银白比武场炉石点以前的跨地图交通不属于33分钟。

固定10项中，《实战训练》就是银白比武场炉石点的第10项；最新原始Questie Journey连续多轮都表现为该轮第一条接13676、最后一条交13676，因此不再把第10项当可变槽。其余九项按用户已经明确的实际顺序执行，不由Timing自行重排。根据飞艇当时位置，允许两种方向：

- 奥尔杜萨侧先走：银白比武场接《实战训练》 → 片刻不得安宁（顺便收集重新考验） → 重新考验 → 盲目的联盟 → 从天而“降” → 地面突袭（顺便做伊米亚之血；不足部分在回程补足） → 空中突袭 → 占山为王 → 萨隆邪铁的奴隶 → 飞船交任务 → 炉石回银白比武场 → 交《实战训练》。
- 地面突袭侧先走：银白比武场接《实战训练》 → 地面突袭 → 空中突袭 → 占山为王 → 萨隆邪铁的奴隶 → 从天而“降” → 盲目的联盟 → 片刻不得安宁（顺便收集重新考验） → 重新考验 → 飞船交任务 → 炉石回银白比武场 → 交《实战训练》。
