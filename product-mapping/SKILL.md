...
8. **处理 KGOS/HEE 新品前先查待补清单** → `data/products/{brand}/pending-products.json`；当前 HEE 6个新品、KGOS 2款七条装咖啡均为“已知待补”，不得归为未知或套用旧版外观。正式简称/主编码需要时查 `data/products/erp-identities.json`，但该200条快照不是在售目录；包装证据与后续固定组合定义指向 `../order-review/SKILL.md`，不从活动简写猜规则。
...
| `data/products/{brand}/pending-products.json` | 已知但缺平台图/稳定视觉资料或装箱资料的在售新品清单；当前 HEE 与 KGOS 均有 | 对应品牌识图、匹配、训练或装箱任务开始前 |
| `data/products/erp-identities.json` | ERP 商品身份只读参考快照（当前200条）：完整名称、正式简称、主商家编码；不是当前商品目录。特殊历史悦希4条另记规格商家编码 | 需要简称/编码核对，或给其他项目补正式 ERP 身份时 |
...
data/products/kgos/pending-products.json
data/products/hee/features.json
data/products/hee/pending-products.json
data/products/erp-identities.json
