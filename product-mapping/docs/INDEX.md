...
`check`、`match`、后置 `check --reuse-active --skip-download` 等确定性 CLI 一旦启动，正常过程由脚本自己负责；Agent 不承担逐 SKU 的实时监工职责。

- **正常运行时禁止模型轮询**：不得为了“看看跑到哪了”周期性读取 DOM、重复 `targets`、tail 日志、读取 `auto-match-log.json`、反复打开运行时 JSON，或在脚本尚未报错时插入额外诊断命令。
- **正常运行时不拆批监工**：不得仅为了让 Agent 持续观察，把原本可一次完成的批处理拆成多个 SKU/小批次执行。
- **只在四种情况重新介入**：进程非 0 退出/抛错；执行环境明确 timeout/SIGTERM；进程成功结束但完成门禁未通过；用户明确要求中途检查。
- **成功路径只做一次终态审计**：命令正常退出后读取最终结构化输出/最新报告，检查本节及 §2 定义的完成门禁；全部通过就结束，不再逐项重复检查 ERP。
- **执行端选择不在本节定义**：ChatGPT + CodexPro 模式的命令路由只看 `docs/chatgpt-codexpro-operations.md`；本节只定义命令启动后的自治/观察边界。
- `auto-match-log.json` 是**异常恢复/最终审计证据**，不是实时监控面板；只有异常终止后判断断点，或任务结束做一次终态审计时读取。
...
正式流程开始前，先由用户手动停止售后系统；CLI 会在正式 `check/match` 入口验证暂停状态。异常或中断后保持停止，只有最终完成门禁全部通过才自动恢复售后。详细恢复语义见 `docs/matching-stability.md §3.9`。
...
- 最终完成必须同时满足 `notInCorr=0`、`recognitionDone=comparisonMatch=SKU总数`、`comparisonMismatch=0`、`comparisonPending=0`、`pendingVisualReview=0`、`unmatchedAwaitingMatch=0`；任一不满足都不能判定完成，也不得恢复售后系统
...
**执行方式**：
- `check.js` 下载 SKU 图片到 `data/imgs/{productCode}__{platformCode}.jpg`，图片路径由商品链接身份推导；报告和 `sku-records.json` 同时保留该 SKU 当前平台原图的 `imgUrl`
- **平台原图中的实际商品实物是识图最高依据**；SKU 名、图片内文字、活动文案只能辅助核对，不能反过来覆盖实物判断
- 本地缓存图、联系表或用户提供的参考图若与 SKU 绑定关系有疑问，必须按该条 `productCode + platformCode` 的 `imgUrl` 重新获取平台原图；参考图默认只说明“商品可能长什么样”，除非用户明确说它就是该 SKU 的绑定真值
- 对照 `data/products/{brand}/features.json` 的颜色、造型、尺寸、版本等视觉特征辅助判断
- 输出结论：商品名称 + 数量 + 置信度（高/低/无法判断）；实物与文字冲突时保留实物结论并单独报告冲突

**识图步骤**：
1. 先加载当前 `productCode + platformCode` 对应的平台原图；若本地图可疑/陈旧，直接用当前 `imgUrl` 重取原图
2. **只看实际实物**：按颜色、造型、瓶/盒形状、尺寸与版本逐一识别图中所有可见商品
3. 数量自检：把图中所有实物逐件计数，与 `recognition.items[].qty` 总和核对；角落、小样、体验装和遮挡物必须扫一遍
4. 最后才读 SKU 名和图片文字做辅助交叉核对；若文字与实物不一致，**不得按文字修正实物识图**，保留实物结果并报告“平台图片/文案绑定不一致”
...
### erp-identities.json — ERP 身份时间点快照

- `data/products/erp-identities.json` 是 2026-09-11 对 ERP 商品档案 V2 当前页做的 200 条只读快照，保存当时的完整名称、正式简称、主商家编码及少量规格商家编码事实。
- 它只用于人工核对或追溯“当时 ERP 身份是什么”，不是当前在售目录，也不是 `features.json` 或 `sku-records.json` 的替代品。
- 现有 `check`、识图、`match` 流程不得因为这份快照存在而增加、删除或自动启用商品；有没有这份文件都不应改变商品匹配的运行结果。

### 新品视觉待办

- 已知新品已经确认 ERP 身份、但商品图或稳定识图特征尚未齐全时，只记录到 `tasks/todo.md`，不提前写入 `features.json`。
- TODO 至少保留标准 ERP 名称、当前编码和仍缺的视觉资料；新版产品不得复用旧版参考图或识图特征。
- 拿到实际商品图后，再按 `docs/brand-onboarding.md` 的原有建档流程补参考图、`features.json` 并完成识图验收。
- 商品匹配 TODO 只负责身份/视觉待办，不作为审单装箱的尺寸、箱规或商品白名单来源。

### 与审单项目的边界

完整ERP名称、商品简称和商家编码必须分字段，名称保留原始空格、全半角括号及符号；不能用简称替代匹配基准。
装箱尺寸、纸箱、零体积配件和固定组合规则由 `order-review` 自己维护；商品匹配项目不因装箱需求修改 `features.json` 或新增结构化商品清单。审单如需判断某个商品是否已完成视觉映射，只读取 `features.json` 条目中的标准 `erpName`，不依赖识图标签、特征、别名或活动 `sku-records`。
...
**不凭EAN-13外观选择查询字段**：纯数字编码也可能是主商家编码。先按主商家编码精确查询；未命中时按规格商家编码回退，并核对返回对象实际包含查询编码。不得仅因数字格式判定字段，也不能把主编码未命中当作档案不存在。
...
- **原图直链渠道**：`sku-records.json` 的 `imgUrl` 来自 ERP 对应表平台图片列，是该 `productCode + platformCode` 当次读取到的平台原图直链。遇到缓存图错误、缩略图看不清、参考图与 SKU 绑定不确定时，优先按这条 `imgUrl` 重取原图；不要靠活动文案猜图
...
- 识图输出首选结构化items；文本使用`erpName×数量`，多商品用中文分号分隔，与§3一致。
...
- `[1/2026-09-21]` **ERP 商品档案V2 路由已迁移**：旧 `#/prod/parallel/` 会导致导航校验失败，当前实测为 `#/prod/parallel_next/`；商品对应表仍为 `#/prod/prod_correspondence_next/`。页面路由升级时先以当前 ERP 实测修正 `lib/navigate.js`，不要把导航失败误判为档案查询逻辑失败。
...
- `[1/2026-04，2026-09-16规则收紧]` **长脚本不要占用受限前台调用，也不要让模型实时监工**：执行端选择与异常恢复见 `docs/chatgpt-codexpro-operations.md`；脚本启动后的观察边界统一遵守 §1「脚本自治边界」，`auto-match-log.json` 只用于异常恢复或终态审计，不做运行中轮询。
...
- `[2/2026-05-20→2026-09-21]` **人工处理某些 SKU 后不能直接续跑 match，必须先重跑 check**：`getTodo()` 的判断条件是 `erpCode === null`，只有 check.js 运行时读 ERP 实时对应表才会回填 erpCode。人工在 ERP 界面完成匹配后，sku-records.json 里该条记录的 erpCode 仍是 null，match 仍视为未匹配并重试，触发重复操作或同样错误。2026-09-21 蘅圆洁面再次触发该场景。正确流程仍是：**人工处理 → check → match**；不得靠手改 `auto-match-log.json` 把人工完成项伪装成脚本 done。
...
- `[2/2026-05-22→2026-09-21]` **识图必须三步走：平台原图实物→数量自检→文字兜底**：只看实物摆放会漏赠品小件，但更危险的是反过来让 SKU 名/图片文字覆盖实际实物。2026-09-21 HEE 收单王实测出现“SKU 名写套组②/③，但该 SKU 绑定的平台原图实际展示另一组”的源数据绑定差异。铁律：①以该 `productCode + platformCode` 的平台原图实物写 recognition.items，口味/规格/是否含8g小样等均以实物为准；②数图中所有商品总件数，与 items.qty 加总对比；③最后才用 SKU 名/图片文字做完整性兜底，不得据文字改写实物结论。实物与文字冲突时保留实物识图并报告源数据不一致。
- `[1/2026-09-21]` **参考图不是 SKU 绑定真值**：用户补充的清晰参考图默认只用于学习某个商品/套组“可能长什么样”，不能拿来决定具体 SKU 对应哪张组合。具体 SKU 的绑定真值只能来自它自己的平台原图；本地图、联系表或参考图有疑问时，按 `sku-records.json imgUrl` 直接重取该 `productCode + platformCode` 原图再判定。
...
- `[4/2026-08-24→2026-09-16]` **确认套件不能用过短门限做一次性判断**：ERP 已接受保存时，“选择商品”弹窗可能稍后才关闭，或先出现“换对应商品”。确认后最多轮询 60 秒；任一状态出现都表示请求已被接收，超时才停止。2026-09-16 茗瑞连续两个套件在旧 30 秒上限后保存成功且回读一致，因此延长为 60 秒；异常停止和后置回读门禁保持不变。
- `[3/2026-09-16→2026-09-21]` **换绑成功后可能残留空的“换对应商品”弹窗**：候选表已清空、ERP 绑定已成功，但旧 wrapper 仍可见，会遮在下一条新打开的“选择商品”上方。每个 SKU 开始前按标题依次关闭残留“换对应商品”和“选择商品”，确认两层均隐藏后再继续；禁止取第一个可见 dialog 猜当前步骤。2026-09-21 蘅圆连续出现 10 秒门限误停，已将残留弹窗关闭等待改为 15 秒；超时仍按 fail-fast 停止，不静默跨过。
