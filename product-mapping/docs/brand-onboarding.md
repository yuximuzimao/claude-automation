...
新品已进入 ERP、但商品图或稳定条码还没齐时，不要直接写 `features.json`，也不要让后续项目把它归为未知：

1. 先写入 `data/products/{brand}/pending-products.json`，至少保存 ERP 名称、简称、当前编码和缺失项。
2. 识图、训练或审单任务命中名称、简称、当前编码任一项时，显示为“已知待补”，停止自动视觉判断或装箱归类。
3. 新版商品不得借用旧版参考图；临时或已知错误的条码不得进入正式映射、训练标签和装箱白名单。
4. 商品图、ERP 精确名称、稳定条码和所需尺寸齐全后，再按下述 Step 3～Step 6 验收并迁入正式资料；迁移完成后从待补清单删除。
