...
- [ ] **HEE 6 个新品补图片与识图特征**
  - `HEE悦希淡纹润护精华口红 09古堡邂逅 3.2g`（简称：悦希口红 09古堡邂逅；编码 `6950328226245`）
  - `HEE悦希玻色因抗皱紧致焕颜面霜（3.0） 50g`（简称：焕颜霜3.0；编码 `6950328225903`）；不得复用面霜2.0视觉。
  - `HEE悦希舒缓焕颜精华乳（2.0） 100g`（简称：焕颜乳2.0；编码 `6940079096228-1`）；该编码已于2026-09-08确认，不再等待更换；不得复用旧舒缓精华乳视觉。
  - `HEE悦希舒缓焕颜精萃水（2.0） 120ml`（简称：焕颜水2.0；编码 `6931583616811`）；不得复用旧舒缓精粹水视觉。
  - `HEE悦希云朵净卸慕斯油 50ml 体验装`（简称：慕斯油 体验装；编码 `6970240607953`）
  - `HEE悦希云朵净卸慕斯油 150ml`（简称：慕斯油；编码 `6970240607946`）
  - 以上只记录商品匹配真正需要的身份与视觉待办；拿到实际商品图后按 `docs/brand-onboarding.md` 完成参考图、`features.json` 与识图验收。装箱尺寸、箱型适用范围和固定组合由 `order-review` 自己维护。

- [x] **KGOS 两款七条装咖啡已补图片与识图特征（2026-09-16）**
  - 生椰拿铁咖啡七条装：ERP 标准名称 `甘油二酯咖啡固体饮料（生椰拿铁味） 8g*7袋/盒`，商家编码 `6980319670030`。
  - 美式咖啡七条装：ERP 标准名称 `KGOS甘油二酯咖啡固体饮料(美式咖啡风味) 5g*7袋/盒`，商家编码 `6980319670023`。
  - 已从用户提供的实际活动图中分别选取清晰单盒图，补入 `data/products/kgos/美式咖啡七条装.jpg`、`生椰拿铁七条装.jpg` 和 `features.json`；识别时以包装口味文字及净含量 `5g×7` / `8g×7` 为关键判据，不复用旧 12 条装咖啡特征。
