...
- **修正后净态原则**：用户明确否定、删除或纠正某个内容后，后续正常产物只描述当前正确状态，不继续围绕被删内容写否定式“幽灵说明”。标题、正文、注释、变量名、PR/commit说明、任务备注等，不得为了证明“已经修了”而反复写“没有X / 不需要X / 已删除X / 不再做X”，除非这条否定信息本身会直接改变当前执行或判断。需要防复发时，把错误模式写进专门的规则、ERROR-BOOK/known pitfalls、测试或审计，不把被删除的错误项继续带回正常交付物。自检：删掉这句关于旧错误/旧内容的否定说明后，如果不影响理解当前正确结果，就删掉。
