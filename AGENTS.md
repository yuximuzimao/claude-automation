...
- `AGENTS.md` / `CLAUDE.md` / 项目 `SKILL.md` 是开工前的预防性入口；`neat-freak` 是阶段收尾兜底，必须在全文文档同步之外反向审计项目是否偏离这些结构规范，包括职责混装、渐进式加载失效、重复权威、旧入口残留和Git忽略误伤。`systematic-debugging` 和 `verification-before-completion` 用于 bug 和完成前验证；`pua` 只在反复失败、被动等待或用户明确触发时使用。
