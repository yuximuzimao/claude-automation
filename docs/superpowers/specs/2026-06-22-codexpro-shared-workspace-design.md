...
- 启动入口只在原有 `codexpro start` 命令上追加 `--allow-root /Users/chat/.config/superpowers/worktrees`；不把整个 `/Users/chat/.config` 作为允许根。
...
- 启动入口：固定 `/Users/chat/claude` 为启动 cwd，并追加 worktree 附加根
...
3. `server_config.allowedRoots` 显示 `/Users/chat/claude` 与 `/Users/chat/.config/superpowers/worktrees`，且不把整个 `/Users/chat/.config` 作为允许根。
