...
审单实验与新品资料：2026-09-08 用户明确三维装箱属于实验功能，独立模拟器/浮窗实验区与历史回放方案仍在讨论，见 `order-review/docs/CURRENT.md`。两款新版咖啡7袋/盒、80盒/箱，待编码及单盒尺寸；焕颜乳2.0编码已确认为 `6940079096228-1`，撤销无效待更换标记。用户确认半毫米向上取整，六款悦希原始尺寸与计算尺寸均保存在 `order-review/data/packing-dimensions.json` 的 `pendingMappings`，待补索引已同步。悦希箱型范围仍待确认；未实现模拟器、改变旧咖啡规则或开放新品箱型白名单。
