...
售后工单倒计时解析已在 2026-07-23 再次修复：Step 10 改为从卡片 `.el-timer` 组件取值，不再按“后自动退款 / 后供应商处理超时 / 后流转至客服”等后缀白名单定位；固定批次失败会写入对应账号的 `error/expired` 状态，店铺管理页持续展示具体异常，不再只依赖临时队列 toast。
